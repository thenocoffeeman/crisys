...
<?php 
		if($patients->result() == null){
            ...
            <tr><td><?=$row->FirstName?> <?=$row->LastName?></td><td><?=$row->Username?></td><td><input type="hidden" name="id" value="<?=$row->ID?>" /><input type="submit" value="View EHR"/></td><td style="color:<?=$color?>"><?=$primary?></td></tr>
            </form>
